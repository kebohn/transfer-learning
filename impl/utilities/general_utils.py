...
def parse_arguments():
  ...
  parser.add_argument('--d', type=utilities.dir_path, help='Directory where files are located (absolute dir)')
  parser.add_argument('--d_test', type=utilities.dir_path, help='Directory where test files are located (absolute dir)')
  parser.add_argument('--d_valid', type=utilities.dir_path, help='Directory where validation files are located (absolute dir)')
  parser.add_argument('--model', type=utilities.dir_path, help='Directory where model is located (absolute dir)')
  parser.add_argument('--features', type=utilities.dir_path, help='Directory where features are located (absolute dir)')
  ...
  parser.add_argument('--knn', dest='knn', action='store_true', help='Apply kNN metric')
  parser.add_argument('--svm', dest='svm', action='store_true', help='Apply Support Vector Machine')
  parser.add_argument('--k', type=int, dest='k', default=5, help='Define k for kNN algorithm (Default: 5)')
  ...
  parser.add_argument('--early-stop', dest='early_stop', action='store_true', help='Define if training should be stopped when plateau is reached, it uses Validation loss per default (Default: false)')
  parser.add_argument('--auc', dest='auc', action='store_true', help='Area under the curve scheme for early stopping, if false validation loss will be used - early-stop variable must be set to true (Default: false)')
  parser.add_argument('--adaptive', dest='adaptive', action='store_true', help='Use adaptive network scheme')
  parser.add_argument('--pretrain', dest='pretrain', action='store_true', help='Use pretrained network scheme')
  parser.add_argument('--finetune', dest='finetune', action='store_true', help='Use fintuned network scheme')
